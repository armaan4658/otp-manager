...
export const UrlOtp = () => {
    ...
    const getData = () => {
        setLoading(true);
        axios.get(`https://url-shortner-back-end-ak.herokuapp.com/otp/get`)
        .then(res=>{
            if(res.status===200){
                setData(res.data);
                setLoading(false);
    ...
    const columns = [
      ...
      const deleteOtp = (delId) => {
          setLoading(true);
          axios.delete(`https://url-shortner-back-end-ak.herokuapp.com/otp/delete/${delId}`)
          .then(res=>{
